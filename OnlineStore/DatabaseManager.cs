...
namespace OnlineStore
{
    public class DatabaseManager
    {
        ...
        public IEnumerable<ProductsModel> GetProductsbyCategory(int? categoryId)
        {
            var CategoryById = webShopDB.Categories
                                          .Include(c => c.Products)
                                          .SingleOrDefault(c => c.CategoryId == categoryId);

            var products = CategoryById.Products.Select(products => new ProductsModel
            {
                ProductId = products.ProductId,
                Name = products.Name,
                Description = products.Description,
